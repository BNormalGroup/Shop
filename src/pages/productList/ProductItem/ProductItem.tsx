            ...
            {loading &&
                ...
                <>
                    <div className={classes.imageContainer}>
                        <img className={classes.image} src={APP_ENV.UPLOADS_URL + item?.items_data.images[0]?.url}
                         alt={''}/>
                        <button className={classes.likeBtn}><img src={likeImage}/></button>
                    </div>
                    <p className={classes.title}>{item?.items_data.product.name}</p>
                    <p className={classes.price}>₴{item?.items_data.product.price} грн.</p>
