...
const ProductListPage = () => {
    ...
    const loadProducts = async (type: string) => {
        if (params.id != undefined) {
            const newProducts = await GetItemListService(params.id, page, sortField, sortDirection);
            if (type == 'page')
    ...
    return (
        <>
            ...
            <div
                className="container"
                style={{marginBottom: 56, padding: 0, minHeight: 1000,display: 'flex', flexDirection: 'column'}}
            >
                <ProductSortPanel setSortField={setSortField} setSortDirection={setSortDirection}></ProductSortPanel>
