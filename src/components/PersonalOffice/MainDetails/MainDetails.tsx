...
const MainDetails = () => {
    ...
    const [user, setUser] = useState<IUserLogin>(useSelector((state: RootState) => state.users.user));
    ...
    async function handleSumbit(event: React.FormEvent) {
        ...
        const responseStatus = await UpdateUserService(user, localStorage.getItem('authToken') ?? '');
    ...
    return (
        <form action="" onSubmit={handleSumbit} className={classes.container}>
            ...
            <div className={classes.inputContainer}>
                ...
                <input className={classes.input} id="firstName" type="text" name='firstName' value={user.firstName}
                       onChange={handleChange}></input>
            </div>
            <div className={classes.inputContainer}>
                <label className={classes.inputLabel} htmlFor="lastName">Last Name</label>
                <input className={classes.input} id="lastName" name='lastName' type="text" value={user.lastName}
                       onChange={handleChange}></input>
            </div>
            <div className={classes.inputContainer}>
                <label className={classes.inputLabel} htmlFor="email">Email Address</label>
                <input className={classes.input} id="email" type="email" name='email' value={user.email}
                       onChange={handleChange}></input>
            </div>
            <InputBirthday birthday={user.birthday} birthdayChange={birthdayChange}></InputBirthday>
