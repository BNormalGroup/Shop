          ...
          <>
            <tr key={item.id} style={{ paddingTop: "20px" }}>
              ...
              <td className={classes.td}>
                {item.delivery_address?.country} | {item.delivery_address?.city}{" "}
                | {item.delivery_address?.streetAddress}
              </td>
              <td className={classes.td}>{item.delivery_address?.email}</td>
              <td className={classes.td}>
                {item.delivery_address?.lastName}{" "}
                {item.delivery_address?.firstName}
              </td>
              <td className={classes.td}>
                {item.delivery_address?.phoneNumber}
              </td>
            </tr>
            <tr>
              <td colSpan={6}>
                <table
                  className={`${classes.table} ${classes.backgroundSecond}`}
                >
                  <thead>
                    <tr>
                      <th
                        className={`${classes.th} ${classes.backgroundSecond}`}
                      >
                        Item Img
                      </th>
                      <th
                  ...
                  <tbody>
                    {item?.item.map((product) => (
                      <tr key={product.id}>
                        ...
                        <td
                          className={`${classes.td} ${classes.backgroundSecond}`}
                        >
                          {product.item?.name}
                        </td>
                        <td
